...
| Communication (News, Notifications, Feedback) | `contracted` | News/Notifications `implemented`; Feedback `partial` | News CRUD and Notifications admin list/detail/create/update plus app reads are live with DTO validation, permissions, audit, pagination, BFF, UI, and tests. Feedback still needs a complete admin detail/update contract and negative tests. |
